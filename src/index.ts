...
export type LanguageCode = string; // expand later to be just those languages that are supported
export type LessonStatus = string; // expand later to be just those statuses that are supported
export type ParamsLessonCreate = {
    description: string;
    groups: unknown[];
    hasPrice: boolean;
    isProtected: boolean;
...
export type LessonCreateReturnType = {
    id: number;
    contentId: number;
    collectionId: number;
...
export default class LingqAPI {
    ...
    async lessonGet(lessonCode?: number): Promise<Lesson> {
        ...
        if (!response.ok) throw new Error("Lesson doesn't exist.");
    ...
    async lessonDelete(lessonId: number): Promise<Response> {
        return fetch(`https://www.lingq.com/api/v2/contexts/7198304/lesson/`, {
            // the context indicates which webpack function it's using I think
            headers: this.buildHeaders({ isPost: true, includeCsrf: true }),
     ...
     */
    async lessonStatsReadTimeIncrement(
     ...
     * @param app Optional parameter if you want to convey the source to be different than the web.
     */
    async lessonStatsListenTimeIncrement(
        incrementAmount: number,
        automatic = true,
        app = 'web'
    ): Promise<LessonStats> {
    ...
    async sentenceTimestampUpdate(index: number, timestamp: [number, number]): Promise<Sentence> {
        ...
        if (!response.ok) throw new Error('Failed to update sentence timestamp');
    ...
    async getTTSSpeech(
        text: string,
        voice: string,
        extraOptions?: { appName?: string; language?: string }
    ) {
